...
def check_numbers_and_return_id(text):
    print(f"\nProcessing line: {text}")
    game_id_match = re.search(r'Game (\d+)', text)
    if not game_id_match:
        print("Game ID not found. Skipping line.")
        return 0

    game_id = int(game_id_match[1])
    print(f"Extracted Game ID: {game_id}")

    pattern = r'(\d+)\s*(red|blue|green)'
    matches = re.findall(pattern, text)
    print(f"Color matches found: {matches}")
    ...
    for number, color in matches:
        if int(number) > color_limits[color]:
            print(
                f"Condition not met for {color} ({number}). Skipping game ID.")
            return 0

    print(f"Game ID {game_id} meets all conditions.")
    return game_id


def sum_applicable_ids(input_data):
    lines = input_data.split("\n")
    total = sum(check_numbers_and_return_id(line)
                for line in lines if line.strip())
    print(f"\nTotal Sum of Applicable Game IDs: {total}")
    return total


input_data = """Game 1: 10 green, 9 blue, 1 red; 1 red, 7 green; 11 green, 6 blue; 8 blue, 12 green
...
total_sum = sum_applicable_ids(input_data)
print(total_sum)
