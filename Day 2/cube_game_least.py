...
def find_least_cubes(line):
    print(f"\nProcessing line: {line}")
    red_numbers, blue_numbers, green_numbers = [], [], []

    color_lists = {'red': red_numbers,
                   'blue': blue_numbers, 'green': green_numbers}

    for color in color_lists.keys():
        pattern = r'(\d+)\s*' + color
...
def process_multiple_lines(input_data):
    lines = input_data.split("\n")
    total_sum = sum(find_least_cubes(line) for line in lines if line.strip())
...
total = process_multiple_lines(input_data)
